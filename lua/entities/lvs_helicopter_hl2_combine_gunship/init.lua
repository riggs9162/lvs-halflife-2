...
function ENT:OnSpawn(PObj)
	...
	if IsValid(self._spotlight) then
		...
		self._spotlight:SetKeyValue("spotlightlength", "1024")
		self._spotlight:SetKeyValue("spotlightwidth", "512")
...
function ENT:OnTick()
	...
	if not self:IsEngineDestroyed() then self:SetRotor(PhysRot) end
	self:AnimBody()
	self:BodySounds()
end
...
function ENT:BodySounds()
	...
	if self._oldHPsnd != HP then
		if isnumber(self._oldHPsnd) and self._oldHPsnd > HP and math.abs(self._oldHPsnd - HP) > 100 then self:EmitSound("NPC_CombineGunship.Pain") end
		self._oldHPsnd = HP
	end
