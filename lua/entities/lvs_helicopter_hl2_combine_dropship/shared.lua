...
ENT.IconOverride = "materials/entities/lvs/hl2/combine_dropship.png"

ENT.AITEAM = 1
ENT.AdminSpawnable = false
ENT.DisableBallistics = true
ENT.GibModels = {"models/container_chunk01.mdl", "models/container_chunk02.mdl", "models/container_chunk03.mdl", "models/container_chunk04.mdl", "models/container_chunk05.mdl"}
ENT.MDL = "models/combine_dropship_container.mdl"
ENT.MaxHealth = 6000
ENT.MaxVelocity = 1750
ENT.SpawnNormalOffset = 64
ENT.Spawnable = true
...
function ENT:InitWeapons()
	...
	weapon.OnDeselect = function(ent) ent:EmitSound("physics/metal/weapon_impact_soft3.wav", nil, 90) end
	...
	weapon.OnThink = function(ent, active) ent:SetPoseParameterTurret() end
	self:AddWeapon(weapon)
	weapon = {}
	...
	weapon.OnThink = function(ent)
		if not ent.deploySound then ent.deploySound = CreateSound(ent, "NPC_CombineDropship.DescendingWarningLoop") end
		if ent.deploySound and not ent.deploySound:IsPlaying() then ent.deploySound:PlayEx(0, 100) end
		...
		if trace.Hit and not ent:GetDoor() and ent:GetDeploying() then
			...
			ent:SetDoor(true)
		elseif (not trace.Hit and ent:GetDoor()) or (not ent:GetDeploying() and ent:GetDoor()) then
