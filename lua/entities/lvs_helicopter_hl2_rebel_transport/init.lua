...
function ENT:OnSpawn(PObj)
	...
	for num, v in pairs(PassengerSeats) do
		...
		if num == 1 then self:SetGunnerSeat(Pod) end
	end
...
function ENT:OnTick()
	...
	if not self:IsSteeringDestroyed() then self:SetTailRotor(PhysRot) end
	if not self:IsEngineDestroyed() then self:SetRotor(PhysRot) end
end

function ENT:OnEngineActiveChanged(Active)
	if Active then self:EmitSound("lvs/vehicles/helicopter/start.wav") end
end
